/****************************************************
    ...
    The main module with all interaction
****************************************************/
...
float test_scenario[][4] = { { 7.5, 140, 0, 0 }, { 5, 40, -0.25 / 8, 0 }, { 0.5, 20, 0, 1.0 }, { 5, 60, 0.25 / 8, 0 }, { 7, 100, 0, 0 } };
...
map<int, long> other_vehicles_last_update_clock;
...
DWORD WINAPI ReceiveThreadFun(void *ptr)
{
	...
	while (1)
	{
		...
		if ((if_shadow) || (frame.iID != my_vehicle->iID))          // jeœli to nie mój w³asny obiekt
		{
			...
			other_vehicles_last_update_clock[frame.iID] = clock();
...
void InteractionInitialization()
{
	...
	multi_reciv = new multicast_net("224.12.13.192", 10001);      // obiekt do odbioru ramek sieciowych
	multi_send = new multicast_net("224.12.13.192", 10001);       // obiekt do wysy³ania ramek
...
void VirtualWorldCycle()
{
	...
	{                              // nale¿y na nowo obliczyæ œredni czas cyklu avg_cycle_time
		...
		sprintf(text, "WZR 2025/26, tem.3, wersja f, czêstoœæ pr.wys. = %0.2f[r/s]  œr.odl = %0.3f[m]  œr.ro¿n.k¹t. = %0.3f[st]",
	...
	int       nCmdShow)
{
	...
	window_handle = CreateWindow(class_name, "WZR 2025/26 temat 3: Nawigacja obl., wersja f", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
		100, 50, 1000, 550, NULL, NULL, hInstance, NULL);
