...
void InteractionInitialisation()
{
	...
	multi_reciv = new multicast_net("224.12.12.12", 10001);      // obiekt do odbioru ramek sieciowych
	multi_send = new multicast_net("224.12.12.12", 10001);       // obiekt do wysy³ania ramek
...
void VirtualWorldCycle()
{
	...
	// --- Spawn positioning: runs once, waits until at least one other car is known ---
	if (!my_car_spawned)
	{
		EnterCriticalSection(&m_cs);
		if (!other_cars.empty())
		{
			// record when we first saw another car
			if (spawn_settle_start == 0)
				spawn_settle_start = clock();

			// wait for the settle period so the receive thread can collect all cars
			bool settled = (clock() - spawn_settle_start) >= spawn_settle_ms * CLOCKS_PER_SEC / 1000;

			if (settled)
			{
				// collect valid car positions
				...
				float best = ref.x*ref.x + ref.z*ref.z;
				for (auto &p : positions)
				{
					float d2 = p.x*p.x + p.z*p.z;
					if (d2 < best) { best = d2; ref = p; }
				}

				// try random angles on a circle of radius spawn_distance around ref
				// until the candidate is at least spawn_distance from every known car
				Vector3 spawn = ref;
				const int max_tries = 36;
				for (int i = 0; i < max_tries; i++)
				{
					// evenly spread angles + a small random jitter so retries vary
					...
					candidate.y = ref.y; // height is corrected by simulation on first frame

					// check candidate is at least spawn_distance from every known car
					bool ok = true;
					for (auto &p : positions)
					{
						float dx = candidate.x - p.x, dz = candidate.z - p.z;
						if (sqrtf(dx*dx + dz*dz) < spawn_distance) { ok = false; break; }
					}
					if (ok) { spawn = candidate; break; }
				}

				my_car->state.vPos = spawn;
				my_car_spawned = true;
			}
		}
		else if (number_of_cyc > 100)
		{
			// this is the first car on the network, keep constructor position
			my_car_spawned = true;
		}
...
LRESULT CALLBACK WndProc(HWND main_window, UINT message_code, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case VK_UP:
		{
			my_car->F = 200.0;        // si³a pchaj¹ca do przodu
			break;
		}
		case VK_DOWN:
		{
